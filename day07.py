...
def part2(_codes, verbose=False):
    ...
    for phases in itertools.permutations(range(5, 10)):
        ...
        states = dict()
        for phase in phases:
            output, __codes, point, input_counter = stateful_intcode_computer(
                _codes, phase, input_signal, verbose=verbose
            )
...
def stateful_intcode_computer(
    _codes, phase_setting=None, input_signal=None, point=0, input_counter=0, verbose=False
):
    codes = _codes.copy()
    while True:
        instruction = codes[point]
